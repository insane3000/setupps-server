...
export const updateComponent: RequestHandler = async (req: any, res) => {
  const componentUpdated = await Power.findByIdAndUpdate(req.body._id, req.body, {
    new: true,
  });
...
export const deleteComponent: RequestHandler = async (req, res) => {
  const componentFound: any = await Power.findByIdAndDelete(req.params.id);
...
export const getComponents: RequestHandler = async (req: any, res) => {
  ...
  try {
    const components = await Power.paginate(
      {
        $or: [
...
export const getComponent: RequestHandler = async (req, res) => {
  ...
  try {
    const component = await Power.findById(id);
